...
<div class="relative h-auto bg-[#eeeeee] pt-10 pb-10">
    <div class="w-5/6 m-auto bg-white  pb-24">
        <form action="{{ route('respuesta_formularios') }}" method="post" class="p-6 pb-4 bg-white w-[95%] h-auto m-auto relative top-10 text-sm" enctype="multipart/form-data">
            @csrf
            ...
            <input type="hidden" name="id_form" value="{{ $form->id }}">
                <br>
                <h1 class="uppercase text-center font-bold text-lg p-0 " style="white-space: pre-wrap;">{{ $form->titulo   }}</h1><br>
            ...
            <div class="flex flex-col  pt-6 px-16">
                ...
                @case('date')
                <input type="date" name="respuesta[{{ $pregunta->id }}]" class="form-control w-full">
                @break

                @case('time')
                <input type="time" name="respuesta[{{ $pregunta->id }}]" class="form-control w-full">
                @break

                @case('email')
                <input type="email" name="respuesta[{{ $pregunta->id }}]" class="form-control w-full">
                @break
                ...
                <div>
                    @foreach ($preguntasDependientes as $select)
                    @if ($select->id_question == $pregunta->id)
                    <br><input type="checkbox" name="respuesta[{{ $pregunta->id }}][]" id="respuesta[{{ $pregunta->id }}]" value="{{ $select -> texto_selects }}"> {{ $select -> texto_selects }}
                    @endif
                    @endforeach
                </div>
            ...
            <input type="submit" value="Envíar" class="w-[80%] uppercase p-4 ml-[10%] mt-10 bg-[#eeeeee] hover:bg-[#ccc] tracking-wide">
        </form>
