        ...
        <div class="relative h-auto">
            ...
            <div class="z-10 w-4/5 h-48 bg-fixed absolute right-0"> <!--Color de fondo-->
                ...
                <div class="m-auto static w-full h-auto">
                    <form action="{{ route('template.crearFormulario') }}" method="post"
                        class="form p-2 pb-10 bg-white w-[95%] h-auto m-auto relative top-10 text-sm " ondrop=""
                        ...
                        @if ($errors->has('preguntas'))
                            <div class="p-4 mb-4 text-sm text-red-800 rounded-lg bg-red-50">
                                <strong>{{ $errors->first('preguntas') }}</strong>
                            </div>
                        @endif
                        ...
                        <div class="flex flex-col place-items-center ">
                            <textarea name="titulo" id="hs-default-height-with-autoheight-script"
                                class="border-none p-0 pt-2 uppercase w-full text-center font-bold focus:ring-0 resize-none overflow-hidden"> Titulo del formulario </textarea>
                            <textarea name="descripcion" id="hs-default-height-with-autoheight-script2"
                                class="border-none p-0 pb-4 w-full text-sm focus:ring-0 resize-none overflow-hidden">Lorem ipsum dolor sit amet consectetur adipisicing elit. Voluptate quae illo nam, rerum maxime porro dolores magnam cumque ex magni natus voluptatum dolore veritatis ad dignissimos obcaecati facere delectus sapiente?</textarea>
                        </div>
                        ...
                        <div class="contenedor flex flex-col gap-y-6 pt-6" id="formulario">
                            @foreach ($preguntas as $pregunta)
                                <div class="form-group">
                                    <label class="uppercase">
                                        ...
                                        <input type="checkbox" name="preguntas[{{ $pregunta->id }}]"
                                            class="absolute right-5" value="{{ $pregunta->id }}"
                                    ...
                                    @switch($pregunta->tipo_de_pregunta)
                                            ...
                                            <select id="preguntas[{{ $pregunta->id }}]" class="form-control w-full ">
                                                ...
                                                @foreach ($selects as $select)
                                                    @if ($select->id_question == $pregunta->id)
                                                        <option value="{{ $select->texto_selects }}" class="uppercase">
                                        ...
                                        @case('checkbox')
                                            ...
                                            @foreach ($selects as $select)
                                                @if ($select->id_question == $pregunta->id)
                                                    <br><input type="checkbox" id="preguntas[{{ $pregunta->id }}]"
                                                        class="uppercase"> {{ $select->texto_selects }}
                                                @endif
                                            @endforeach
                                        @break
                            ...
                            <div class="w-full h-20 mt-5 border-2 border-dashed border-gray-400 flex items-center justify-center"
                                id="zonaDrop">
                        ...
                        <input type="submit" value="Crear"
                            class="w-full uppercase p-4 mt-6 bg-[#eeeeee] hover:bg-[#ccc] tracking-wide">
            ...
            <div class="fixed sm:w-1/4 lg:w-1/5 w-3/4 bg-white rounded-xl h-4/5 border-4 overflow-y-auto ml-2">
                <div class="flex flex-col p-4 gap-y-4 pt-16 sm:pt-24">
                    <button type="button" id="botonAreaDeTexto" draggable="true"
                        class="w-full h-12 rounded-md bg-gray-100 cursor-move text-center text-sm sm:text-base">
                        Área de texto
                    </button>
                    <button type="button" id="botonTexto" draggable="true"
                        class="w-full h-12 rounded-md bg-gray-100 cursor-move text-center text-sm sm:text-base">
                        Texto
                    </button>
                    <button type="button" id="botonNumber" draggable="true"
                        class="w-full h-12 rounded-md bg-gray-100 cursor-move text-center text-sm sm:text-base">
                        Número
                    </button>
                    <button type="button" id="botonSelect" draggable="true"
                        class="w-full h-12 rounded-md bg-gray-100 cursor-move text-center text-sm sm:text-base">
                        Selección
                    </button>
                    <button type="button" id="botonDate" draggable="true"
                        class="w-full h-12 rounded-md bg-gray-100 cursor-move text-center text-sm sm:text-base">
                        Fecha
                    </button>
                    <button type="button" id="botonCheck" draggable="true"
                        class="w-full h-12 rounded-md bg-gray-100 cursor-move text-center text-sm sm:text-base">
                        Checkbox
                    </button>
                </div>
